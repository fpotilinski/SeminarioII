...
<body>
	...
	<div id="frame">
		<div id="sidepanel">
			...
			<div id="contacts">
				<ul>
					<c:forEach items="${chats}" var="chat" varStatus="loop">
						<li class="contact" id="id${loop.index}"
							data-object="${chat.referencia}">
							<div class="wrap">
								<span class="contact-status online"></span> <img
									src="http://emilcarlsson.se/assets/louislitt.png" alt="" />
								<div class="meta">
									<p class="name">${chat.referencia}</p>
									<p class="preview">${chat.getMensajes().get(chat.getMensajes().size() - 1).getMensaje()}</p>
								</div>
							</div>
						</li>
					</c:forEach>
					...
					<%-- 					<% 
// 						List<ChatDTO> chatsDto = (List<ChatDTO>) request.getAttribute("chats");
// 						ChatDTO chat = null;
// 						for (Iterator<ChatDTO> i = chatsDto.iterator(); i.hasNext();) {
// 							chat = i.next();
<%-- 					%> --%>
					<!-- 					<li class="contact" id="chatMensajes"> -->
					<!-- 						<div class="wrap"> -->
					<!-- 							<span class="contact-status online"></span> <img -->
					<!-- 								src="http://emilcarlsson.se/assets/louislitt.png" alt="" /> -->
					<!-- 							<div class="meta"> -->
					<%-- 								<p class="name"><%=chat.getReferencia()%></p> --%>
					<!-- 								<p class="preview"> -->
					<%-- 									<%= 
// 										chat.getMensajes().get(chat.getMensajes().size() - 1).getMensaje()
<%-- 									%> --%>
					<!-- 								</p> -->
					<!-- 							</div> -->
					<!-- 						</div> -->
...
<%-- 					%> --%>
		...
		<div class="content" id="cabeceraMensajes">
			...
			<div class="messages">
				<ul id="mensajes">
					<c:forEach items="${chats[0].getMensajes()}" var="msj">
						<c:choose>
							<c:when test="${msj.getUsuario().getIdUsuario()==usuario.getIdUsuario()}">
								<li class="sent"><img
			...
			<div class="message-input">
				<div class="wrap">
					<input type="text" placeholder="Escribe tu mensaje..." /> <i
	...
	<script>
		...
		$(".contact").unbind().click(
				function() {
// 					var test = ${chats[0]};
// 					alert(test.referencia);
// 					var mensajesArray = ${chats[1].getMensajes()};
// 					$.each(mensajesArray, function( key, value ) {
...
// 						$('.contact.active .preview').html('<span>You: </span>' + value.mensaje);
// 						$(".messages").animate({
// 							scrollTop : $(document).height()
// 						}, "fast");
...
// 					$("#cabeceraMensajes").find("p").first().text(
// 							$(this).find("p").first().text());
// 					$("#cabeceraMensajes").find("img").attr("src",
